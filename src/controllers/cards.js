...
exports.addToCollection = async (req, res) => {
  try {
    ...
    const cardAlreadyAdded = user.cards.some(userCard =>
      userCard.equals(card._id)
    )
    if (cardAlreadyAdded) {
      return res
        .status(400)
        .json({ message: 'Card already added to collection' })
    }
...
exports.removeFromCollection = async (req, res) => {
  try {
    ...
    const cardIndex = user.cards.findIndex(userCard =>
      userCard.equals(card._id)
    )
    if (cardIndex === -1) {
      return res
        .status(404)
        .json({ message: 'Card not found in user collection' })
    }
...
exports.addToWishlist = async (req, res) => {
  try {
    ...
    const isCardInWishlist = user.wishlist.some(wishlistCard =>
      wishlistCard.equals(card._id)
    )
    if (isCardInWishlist) {
...
exports.removeFromWishlist = async (req, res) => {
  try {
    ...
    user.wishlist = user.wishlist.filter(
      wishlistCardId => !wishlistCardId.equals(card._id)
    )
    await user.save()
